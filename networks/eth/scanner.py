...
from base_types import Block, Scanner, BlockEvent
...
class EthScanner(Scanner):
    ...
    def _check_tx_from(self, tx, addresses):
        ...
        if from_address:
            ...
            logging.info('{}: Empty from field for transaction {}. Skip it.'.format(self.network.type, tx.tx_hash))
    ...
    def _check_tx_creates(self, tx, address):
        ...
        if tx.creates:
            ...
            try:
                ...
                logging.error('{}: Error on getting transaction {} receipt.'.format(self.network.type,
                                                                            tx.tx_hash))
                logging.error('{}: Empty to and creates field for transaction {}. Skip it.'.format(self.network.type,
                                                                                           tx.tx_hash))

    def _find_event(self, block: Block) -> dict:
        find_events = {}
        events = {
            "DepositNFT": exchange_from_ethereum,
        }
